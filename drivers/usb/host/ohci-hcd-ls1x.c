...
static struct ohci_hcca ghcca[1];
/* a pointer to the aligned storage */
static struct ohci_hcca *phcca;
/* this allocates EDs for all possible endpoints */
static struct ohci_device ohci_dev;
static struct ohci_device *pohci_dev;
...
static int ep_link(ohci_t *ohci, ed_t *edi)
{
	...
	case PIPE_CONTROL:
		...
		if (ohci->ed_controltail == NULL)
			ohci_writel(virt_to_phys(ed), &ohci->regs->ed_controlhead);
		else
			ohci->ed_controltail->hwNextED =
						   m32_swap((unsigned long)virt_to_phys(ed));
	...
	case PIPE_BULK:
		...
		if (ohci->ed_bulktail == NULL)
			ohci_writel(virt_to_phys(ed), &ohci->regs->ed_bulkhead);
		else
	...
	case PIPE_INTERRUPT:
		...
		for (i = 0; i < ep_rev(6, interval); i += inter) {
			...
			*(__u32 *)UNCACHED(ed_p) = m32_swap((unsigned long)virt_to_phys(ed));
		}
		break;
	}
	return 0;
	...
	for (; index < NUM_INTS; index += period) {
		...
		while (*(__u32 *)UNCACHED(ed_p) != 0) {
			if (((struct ed *)
				...
				*(__u32 *)UNCACHED(ed_p) = UNCACHED(ed->hwNextED);
				break;
			}
			ed_p = &(((struct ed *)
			...
			int interval, int load)
{
	...
	ed = ed_ret = &pohci_dev->ed[(usb_pipeendpoint(pipe) << 1) |
	...
	if (ed->state == ED_NEW) {
		...
		td = td_alloc(usb_dev);
		ed->hwTailP = m32_swap((unsigned long)virt_to_phys(td));
	...
	struct usb_device *dev, int index, urb_priv_t *urb_priv)
{
	...
	td = urb_priv->td [index] = (td_t *)CACHED_TO_UNCACHED(td);
	...
	td->hwCBP = m32_swap((unsigned long)virt_to_phys(data));
	if (data)
		td->hwBE = m32_swap((unsigned long)virt_to_phys(data + len - 1));
	else
		td->hwBE = 0;

	td->hwNextTD = m32_swap((unsigned long)virt_to_phys(td_pt));
...
static void dl_transfer_length(td_t *td)
{
	...
	if (!(usb_pipecontrol(lurb_priv->pipe) &&
		...
		if (tdBE != 0) {
			if (td->hwCBP == 0)
				lurb_priv->actual_length += PHYS_TO_CACHED(tdBE) - td->data + 1;
			else
				lurb_priv->actual_length += PHYS_TO_CACHED(tdCBP) - td->data;
		}
	}
}
...
static void check_status(td_t *td_list)
{
	...
	__u32      *phwHeadP  = (__u32 *)UNCACHED((unsigned long)&td_list->ed->hwHeadP);
	int	   cc;
...
#define WR_RH_STAT(x)		{info("WR:status %#8x", (x)); ohci_writel((x), \
						&pohci->regs->roothub.status); }
#define WR_RH_PORTSTAT(x)	{info("WR:portstatus[%d] %#8x", wIndex-1, \
	(x)); ohci_writel((x), &pohci->regs->roothub.portstatus[wIndex-1]); }
#else
#define WR_RH_STAT(x)		ohci_writel((x), &pohci->regs->roothub.status)
#define WR_RH_PORTSTAT(x)	ohci_writel((x), \
				    &pohci->regs->roothub.portstatus[wIndex-1])
#endif
#define RD_RH_STAT		roothub_status(pohci)
#define RD_RH_PORTSTAT		roothub_portstatus(pohci, wIndex-1)
		...
		int transfer_len, struct devrequest *setup)
{
	...
	if (((pipe >> 8) & 0x7f) == pohci->rh.devnum) {
		pohci->rh.dev = dev;
		/* root hub - redirect */
...
static int hc_start(ohci_t *ohci)
{
	...
	ohci_writel((__u32)virt_to_phys(ohci->hcca),
...
int usb_lowlevel_init(int index, void **controller)
{
	...
	pohci = (ohci_t *)UNCACHED((unsigned long)&gohci);
#else
	if (!ohci_inited){
		pohci = (ohci_t *)malloc(sizeof(ohci_t));
		if (!pohci) {
	...
	memset(&ghcca[0], 0, sizeof(ghcca[0]));
	flush_cache((unsigned long)&ghcca[0], sizeof(ghcca[0]));
	phcca = (struct ohci_hcca *)UNCACHED((unsigned long)&ghcca[0]);
#else
	phcca = (struct ohci_hcca *)malloc(sizeof(struct ohci_hcca) + 256);
	phcca = (struct ohci_hcca *)ALIGN((unsigned int)phcca, 256);
	phcca = (struct ohci_hcca *)UNCACHED((unsigned long)phcca);
#endif
	info("aligned ghcca %p", phcca);

	memset(&ohci_dev, 0, sizeof(struct ohci_device));
	flush_cache((unsigned long)&ohci_dev, sizeof(struct ohci_device));
	pohci_dev = (struct ohci_device *)UNCACHED((unsigned long)&ohci_dev);
	if ((__u32)&pohci_dev->ed[0] & 0x7) {
		err("EDs not aligned!!");
		return -1;
	}
	...
	ptd = (td_t *)UNCACHED((unsigned long)gtd);
	pohci->hcca = phcca;
	...
	if (pdev != -1) {
		...
		pohci->regs = (struct ohci_regs *)base;
	} else
		return -1;
#else
	pohci->regs = (struct ohci_regs *)CONFIG_SYS_USB_OHCI_REGS_BASE;
#endif

	pohci->flags = 0;
	pohci->slot_name = CONFIG_SYS_USB_OHCI_SLOT_NAME;

	if (hc_reset (pohci) < 0) {
		hc_release_ohci (pohci);
		err ("can't reset usb-%s", pohci->slot_name);
#ifdef CONFIG_SYS_USB_OHCI_BOARD_INIT
	...
	if (hc_start(pohci) < 0) {
		err("can't start usb-%s", pohci->slot_name);
		hc_release_ohci(pohci);
